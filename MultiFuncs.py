...
def n_func(n, known=[]):
    ...
    def _wrapper(func):
        ...
        # make sure that functions like f(x) appear.... what would you want to fit?
        ...
        assert len(known) <= n * \
            Nf, "Number of known parts must lesser than number of functions!"
        ...
        _signature = _signature + \
            [inspect.Parameter(
                _, kind=inspect.Parameter.POSITIONAL_OR_KEYWORD) for _ in nf_args]
...
def n_func_mix(funcs):
    ...
    signature_ = [__ + str(_) for _ in range(len(funcs))
                  for __ in signature_[_] if not 'x' in __]
    _signature = [inspect.Parameter(
        'x', kind=inspect.Parameter.POSITIONAL_OR_KEYWORD)]
    _signature = _signature + \
        [inspect.Parameter(_, kind=inspect.Parameter.POSITIONAL_OR_KEYWORD)
