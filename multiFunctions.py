...
# Decorator
def n_func(n, known=[]):
    '''Decorator for defining functions mixed by base function. For scipy.optimize.curv_fit
    Arguments:
    n: numbers of functions
    Keyword arguments:
    known: optional, known componets of functions, n_{knowns} \times n_{function parameters}
    
    Returns: Function that mixed n base functions with known parameters.
    '''
    def _wrapper(func):
        signature_ = inspect.getfullargspec(func).args
        assert signature_[0] == 'x', "The function must has `x' for its 1st value!" # For scipy fitting
        assert len(signature_[0]) > 1, "........Seriously?" # make sure that functions like f(x) appear.... what would you want to fit?
        f_args = signature_[1:]
        ...
        if not (len(known) != n * Nf or None in known):
            raise ValueError("Seriously?") # I am sure that no parameters are put for fitting
        nf_args = [f_args[_%Nf]+str(_//Nf+Nk//Nf) for _ in range(n*Nf-Nk)] # for f_{1,2,3}(x,a, b), transfrom a -> a0 b0, a1 b1, a2 b2 
        nf_args = [ f_args[_%Nf]+str(_//Nf) for _ in range(Nk) if known[_] is None ] + nf_args # same operation, but for the known part
        #print(nf_args)
        ...
        def __wrapper(x,*args):
            ...
            args = list(args)
            # this is amazing [ None, a, b, None ] + [c,d] -> [c,a,b,d], a fancy substitution
